        ...
        <Modal onClose={() => setShowCreate(false)} title="Create a Workspace">
          <form onSubmit={onCreate} className="space-y-3">
            <div className="grid grid-cols-3 gap-2">
              {(Object.keys(TYPE_META) as WSType[]).map((t) => (
                <button type="button" key={t} onClick={() => setForm((f) => ({ ...f, type: t, emoji: TYPE_META[t].emoji }))}
                  className={`p-3 rounded-2xl border text-center transition-soft ${form.type === t ? "border-primary bg-primary/10" : "border-border/60 hover:bg-muted"}`}>
                  <div className="text-xl">{TYPE_META[t].emoji}</div>
                  <div className="mt-1 text-[11px] font-medium">{TYPE_META[t].label}</div>
                </button>
              ))}
            </div>
...
function EmptyState({ onCreate }: { onCreate: () => void }) {
  const types = [
    { t: "teacher" as const, Icon: GraduationCap, label: "Teacher", desc: "Run classes, assignments, attendance" },
    { t: "student" as const, Icon: BookOpen, label: "Student", desc: "AI tutor, notes, exam prep" },
    { t: "friends" as const, Icon: Users, label: "Friends", desc: "Plans, polls, shared whiteboard" },
    { t: "business" as const, Icon: Building2, label: "Business", desc: "Projects, departments, knowledge" },
  ];
  return (
